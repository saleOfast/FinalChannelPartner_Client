    ...
    const handleImageChange = (e) => {
      ...
      if (file) {
        ...
        reader.onloadend = () => {
          ...
          img.onload = () => {
            // Check dimensions
            if (img.width !== 145 || img.height !== 65) {
              setUserInfo({...userInfo,logo:null,logoPreview:null})
              toast.error('Invalid image dimensions. The image must be 145x65 pixels.',{autoClose:1500});
              e.target.value = ''; // Clear the input value to prevent invalid file from being selected
              return; // Exit the function early
            }
                ...
                {/* for changing apps permission when we click on edit and logo */}
                  ...
                  >
                   ...
                   <div className="mb-3">
                    ...
                    <label className="form-label">Logo (Best Dimension: 145x65)</label>
                      {/* <input
                        type="file"
