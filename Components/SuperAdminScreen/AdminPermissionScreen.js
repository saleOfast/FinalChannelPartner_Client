...
//                         {hasChildren && (
//                             <div className="openToggler" aria-controls={menu.menu_id} aria-expanded={open[menu.menu_id] || false} onClick={() => handleClick(menu.menu_id)}>
//                                 <CaretDownIcon />
//                             </div>
//                         )}
//                         <input
...
//                             onChange={(e) => clickHandler(e, menu.menu_id, menu.menu_name, menu.parent_id, menu.children)}
//                         />
//                         <label htmlFor={`${menu.menu_id}-input`} className="form-check-label">
//                             <span className="check-head">{menu.allais_menu}</span>
//                         </label>
...
//                     {hasChildren && (
//                         <Collapse in={open[menu.menu_id]}>
//                             <div className="child-box row m-auto" id={menu.menu_id}>
//                                 {renderMenu(menu.children)}
//                             </div>
//                         </Collapse>
//                     )}
...
//     const clickHandler = (e, menu_id, menu_name, parent_id, childrens) => {
        
//         const currArr = updateActionsForParentIdThree([...permissionView], menu_id, e.target.checked);
//         const modifiedArray = addArrayToChildren(permissionView, menu_id, childCheckFuncs(childrens, e.target.checked));
//         checkFuncc(currArr);
//     };

//     const updateActionsForParentIdThree = (arr, menu_id, checked) => {
        
//         arr.forEach((item) => {
//             if (item.menu_id === menu_id) {
//                 item.actions = checked;
//                 item.is_active = checked;
//             }
//             if (item.children.length > 0) {
//                 updateActionsForParentIdThree(item.children, menu_id, checked);
//             }
//         });
//         return arr;
//     };

//     const addArrayToChildren = (array, menu_id, data) => {
//         array.forEach((item) => {
//             if (item.menu_id === menu_id) {
//                 item.children = data;
//             }
...
//     const childCheckFuncs = (menu, checked) => {
//         menu?.forEach((child) => {
//             child.is_active = checked;
//             child.actions = checked;

//             if (child.children) {
//                 childCheckFuncs(child.children, checked);
//             }
//         });
//         return menu;
//     };

//     const checkFuncc = (valArr, returnArr = []) => {
//         valArr?.forEach((data) => {
//             const checkinfo = {
//                 menu_id: data.menu_id,
//                 menu_name: data.menu_name,
...
//                         <Form.Group controlId="dropdownMenu">
//                             <Form.Label>Select an option:</Form.Label>
//                             <Form.Control as="select" value={selectedOption} onChange={handleChange} className="form-control-sm">
//                                 <option>CRM</option>
//                                 <option>CHANNEL</option>
//                                 <option>DMS</option>
//                                 <option>SALES</option>
...
//                     {permissionView?.length > 0 && (
//                         <div className="submit-btn-box">
//                             <button className="btn btn-cancel" disabled={isButtonLoading} onClick={() => router.push('/admin')}>
//                                 Go Back
//                             </button>
//                             <button onClick={submitFunc} disabled={isButtonLoading} className="btn btn-primary">
//                                 {isButtonLoading ? (
